                ...
                <div class="portlet-body">
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <button id="sample_editable_1_new" onclick="location.href ='{{route('why-us.create')}}'" class="btn sbold green"> {{trans('admin/blog.add_new')}}
                    ...
                    {!! Form::open(['route' => ['why-us.destroy.all'] , 'method' => 'delete', 'id'=>'form-delete']) !!}
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                        ...
                        @foreach($items as $item)
                            <tr class="odd gradeX">
                                <td>
                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                        <input type="checkbox" name="checkbox[]" class="checkboxes sub_chk" value="{{$item->id}}" data-id="{{$item->id}}" />
                                        <span></span>
                                    </label>
                                </td>
                                ...
                                <td class="text-center">
                                    <span class="label label-sm label-{{$item->status == 0 ? 'danger' : 'success'}}"> {{$item->status == 0 ? 'inactive' : 'active'}} </span>
                                </td>
                                <td class="text-center vcenter">
                                    <a href="{{route('why-us.edit',$item->id)}}" title="{{trans('admin/services.edit')}}"><i class="fa fa-edit"></i></a>
                                    {!! Form::open(['route' => ['why-us.destroy',$item->id] , 'method' => 'delete','style'=>'display: inline','id'=>'Form'.$item->id]) !!}
                                    <a href="javascript:{}" onclick='document.getElementById("Form{{$item->id}}" ).submit();' title="{{trans('admin/services.delete')}}"><i class="fa fa-trash"></i></a>
                                    {!! Form::close() !!}
                                </td>
