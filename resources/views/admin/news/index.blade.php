...
@section('title', trans('admin/school.news'))
...
@section('page_title',trans('admin/school.news'))
            ...
            <div class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        ...
                        <span class="caption-subject bold uppercase"> {{trans('admin/school.news_table')}}</span>
                    </div>
                ...
                <div class="portlet-body">
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <button id="sample_editable_1_new" onclick="location.href ='{{route('news.create')}}'" class="btn sbold green"> {{trans('admin/school.add_new_news')}}
                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                        ...
                        <tr>
                            ...
                            <th>#</th>
                            <th class="text-center"> {{trans('admin/school.news_title')}} </th>
                        ...
                        @foreach($news as $single_new)
                            <tr class="odd gradeX">
                                <td>
                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                        <input type="checkbox" name="checkbox[]" class="checkboxes sub_chk" value="{{$single_new->id}}" data-id="{{$single_new->id}}" />
                                        <span></span>
                                    </label>
                                </td>
                                <td>{{$loop->iteration}}</td>
                                <td class="text-center">
                                    @foreach($single_new->description as $description)
                                ...
                                <td class="text-center vcenter">
                                    <span class="label label-sm label-{{$single_new->home_page_status == 0 ? 'danger' : 'success'}}"> {{$single_new->home_page_status == 0 ? 'inactive' : 'active'}} </span>

                                </td>
                                <td class="text-center">
                                    <span class="label label-sm label-{{$single_new->status == 0 ? 'danger' : 'success'}}"> {{$single_new->status == 0 ? 'inactive' : 'active'}} </span>
                                </td>
                                <td class="text-center vcenter">
                                    <a href="{{route('news.edit',$single_new->id)}}" title="{{trans('admin/services.edit')}}"><i class="fa fa-edit"></i></a>
                                    {!! Form::open(['route' => ['news.destroy',$single_new->id] , 'method' => 'delete','style'=>'display: inline','id'=>'Form'.$single_new->id]) !!}
                                    <a href="javascript:{}" onclick='document.getElementById("Form{{$single_new->id}}" ).submit();' title="{{trans('admin/services.delete')}}"><i class="fa fa-trash"></i></a>
                                    {!! Form::close() !!}
                                </td>
